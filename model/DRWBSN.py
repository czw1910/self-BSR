...
class DRWBSN(nn.Module):
    ...
    def forward(self, x): 
        x_h = self.head(x)     
        if self.refine:
            x_h = F.interpolate(x_h,scale_factor=(1,2),mode='bilinear')                      
        x_mask = self.maskconv_I(x_h)
        if self.refine:
            x_mask = F.interpolate(x_mask,scale_factor=(1/1,1/2),mode='bilinear')  
        ...
        if self.draw_feature:
            ...
            draw_features(8,8,x_mask_pd.cpu().numpy(),self.experimrnt_dir,'x_mask_pd.png')
            draw_features(8,8,x_pd_w.cpu().numpy(),self.experimrnt_dir,'x_pd_w.png')
            draw_features(8,8,x_pd_w_HL.cpu().numpy(),self.experimrnt_dir,'x_pd_w_HL.png')
            draw_features(8,8,I_pd_b_w.cpu().numpy(),self.experimrnt_dir,'I_pd_b_w.png')
            draw_features(8,8,S_pd_b_w_HL.cpu().numpy(),self.experimrnt_dir,'S_pd_b_w_HL.png')  
            draw_features(8,8,I_pd_b_.cpu().numpy(),self.experimrnt_dir,'I_pd_b_.png') 
